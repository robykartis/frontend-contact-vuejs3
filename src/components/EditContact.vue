...
export default {
  ...
  methods: {
    async getContactById() {
      let url = `http://127.0.0.1:8000/api/get_contacts/${this.$route.params.id}`;
    ...
    async updateContact() {
      ...
      if (!this.errors.length) {
        ...
        let url = `http://127.0.0.1:8000/api/update_contacts/${this.$route.params.id}`;
        await axios
          .post(url, formData)
          .then((response) => {
            console.log(response);
  ...
  mounted: function () {
    console.log("Edit Componet Form Loaded...");
  },
};
</script>
